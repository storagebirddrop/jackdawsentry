...
class CaseManagementEngine:
    ...
    async def create_case(
        ...
        try:
            ...
            if not due_date:
                total_duration = sum(
                    (
                        step.get("estimated_duration", timedelta(days=1))
    ...
    async def update_case_status(
        self, case_id: str, new_status: CaseStatus, updated_by: str, notes: str = None
    ) -> bool:
        """Update case status"""
        try:
    ...
    async def advance_case_workflow(
        self, case_id: str, advanced_by: str, notes: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not workflow:
                raise ValueError(
                    f"No workflow found for case type {case.case_type.value}"
                )

            current_step_index = case.current_step
            if current_step_index >= len(workflow.steps):
                return {"success": False, "message": "Case is already at final step"}
            ...
            for req_evidence in required_evidence:
                if not any(
                    ev.evidence_type.value == req_evidence for ev in case_evidence
                ):
        ...
        try:
            ...
            if workflow:
                workflow_progress = {
                    ...
                    "total_steps": len(workflow.steps),
                    "current_step_name": (
                        workflow.steps[case.current_step]["name"]
                        if case.current_step < len(workflow.steps)
                        else "Completed"
                    ),
                    "progress_percentage": (
                        (case.current_step / len(workflow.steps)) * 100
                        if workflow.steps
                        else 0
                    ),
                }
            ...
            metrics = {
                ...
                "days_until_due": (
                    (case.due_date - datetime.now(timezone.utc)).days
                    if case.due_date
                    else None
                ),
            ...
            for ev in evidence:
                ...
                metrics["evidence_by_type"][ev_type] = (
                    metrics["evidence_by_type"].get(ev_type, 0) + 1
                )
            ...
            return {
                ...
                "evidence": {
                    ...
                    "recent_evidence": [
                        ...
                        for ev in sorted(
                            evidence, key=lambda x: x.collected_at, reverse=True
                        )[:5]
                    ],
                },
                "metrics": metrics,
            ...
            async with get_neo4j_session() as session:
                ...
                if record:
                    ...
                    return Case(
                        ...
                        regulatory_jurisdictions=data.get(
            ...
            async with get_neo4j_session() as session:
                ...
                await session.run(
                    query,
                    {
                        ...
                        "archived_at": (
                            evidence.archived_at.isoformat()
