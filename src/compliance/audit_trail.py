...
class AuditTrailEngine:
        ...
        self.compliance_requirements = {
            ComplianceCategory.GDPR: [
                ...
                },
                {
                    ...
                    'audit_events': ['security_breach', 'privilege_escalation', 'system_config_change'],
            ...
            ComplianceCategory.AML: [
                {
                    ...
                    'audit_events': ['case_created', 'evidence_added', 'report_generated'],
                ...
                },
                {
                    ...
                    'audit_events': ['case_created', 'report_submitted', 'regulatory_filing'],
            ...
            ComplianceCategory.SOX: [
                {
                    ...
                    'audit_events': ['report_generated', 'system_config_change', 'data_export'],
                    'retention_period': timedelta(days=2555)
                }
            ]
        }
    ...
    async def log_event(self,
        ...
        try:
            ...
            if self.chain_integrity_enabled:
                ...
                event.previous_hash = previous_event.event_hash if previous_event else None
    ...
    async def verify_chain_integrity(self, start_date: datetime = None, end_date: datetime = None) -> Dict[str, Any]:
    ...
    async def generate_audit_report(self,
        ...
        try:
            ...
            for event in events:
                ...
                for category in event.compliance_categories:
                    if category.value not in compliance_status:
                        compliance_status[category.value] = {'compliant': 0, 'non_compliant': 0, 'total': 0}
            ...
            # Security events
            security_events = [e for e in events if e.event_type in [AuditEventType.SECURITY_BREACH, AuditEventType.PRIVILEGE_ESCALATION]]
            if security_events:
                findings.append({
            ...
            if severity_counts[AuditSeverity.CRITICAL] > 0:
                recommendations.append("Review and address all critical events immediately")
            ...
            if len(user_activity) > 100:
                recommendations.append("Review user access patterns and implement principle of least privilege")
            ...
            logger.info(f"Generated audit report {report_id} for period {period_start} to {period_end}")
    ...
    async def get_compliance_status(self, category: ComplianceCategory = None) -> Dict[str, Any]:
        ...
        try:
            async with get_neo4j_session() as session:
                ...
                await session.run(query, {
                    ...
                    'compliance_categories': [cat.value for cat in event.compliance_categories],
    ...
    async def _get_latest_event(self) -> Optional[AuditEvent]:
        ...
        try:
            async with get_neo4j_session() as session:
                query = "MATCH (e:AuditEvent) RETURN e ORDER BY e.timestamp DESC LIMIT 1"
                ...
                if record:
                    ...
                    return AuditEvent(
                        ...
                        compliance_categories=[ComplianceCategory(cat) for cat in data.get('compliance_categories', [])],
                        retention_period=timedelta(seconds=data.get('retention_period', 0)),
    ...
    async def _get_events_for_verification(self, start_date: datetime = None, end_date: datetime = None) -> List[AuditEvent]:
    ...
    async def _get_events_by_period(self, start_date: datetime, end_date: datetime, filters: Dict[str, Any] = None) -> List[AuditEvent]:
    ...
    async def _get_category_compliance_status(self, category: ComplianceCategory) -> Dict[str, Any]:
        ...
        try:
            ...
            for req in requirements:
                ...
                events_count = len(await self.search_audit_events(
