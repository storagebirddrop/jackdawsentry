...
class ComplianceScheduler:
    ...
    def _setup_default_tasks(self):
        ...
        self.add_task(
            ...
            metadata={
                "report_types": ["summary", "risk_assessment", "case_statistics"]
            },
        )
    ...
    def _schedule_task(self, task: ScheduledTask):
        ...
        try:
            ...
            if task.frequency == TaskFrequency.MINUTELY:
                ...
                task.function = schedule.every(interval).minutes.do(
            ...
            # Set next run time
            task.next_run = (
                schedule.next_run() if hasattr(schedule, "next_run") else None
            )
    ...
    async def _execute_task(self, task: ScheduledTask):
        ...
        try:
            ...
            if task.error_count < task.max_retries:
                logger.info(
                    f"Retrying task {task.name} (attempt {task.error_count + 1}/{task.max_retries})"
                )
                ...
                logger.error(
                    f"Task {task.name} failed after {task.max_retries} attempts"
                )
                task.status = TaskStatus.FAILED
    ...
    async def _monitor_deadlines(self) -> Dict[str, Any]:
        ...
        try:
            ...
            upcoming_deadlines = await router.regulatory_engine.get_upcoming_deadlines(
                hours_ahead=72
            )
            ...
            logger.info(
                f"Deadline monitoring completed: {len(results['upcoming_deadlines'])} upcoming, {len(results['overdue_deadlines'])} overdue"
            )
            return results
    ...
    async def _cleanup_expired_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cleanup_stats = await schema_manager.cleanup_old_data(
            ...
            results["cache_entries_deleted"] = cache_cleanup_stats.get(
                "total_cleaned", 0
            )
    ...
    async def _generate_daily_reports(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {"reports_generated": [], "errors": []}
            ...
            logger.info(
                f"Daily report generation completed: {len(results['reports_generated'])} reports, {len(results['errors'])} errors"
            )
            return results
    ...
    async def _database_maintenance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {"operations_completed": [], "errors": []}
            ...
            # Optimize database
            await session.run(
                "CALL dbms.stats.retrieve('GRAPH NAME')"
    ...
    async def _send_deadline_reminder(self, deadline: Dict[str, Any]):
        ...
        logger.warning(
            f"Deadline reminder: {deadline['report_id']} due in {deadline.get('hours_until', 'unknown')} hours"
        )
    ...
    async def _create_overdue_deadline_alert(self, deadline: Dict[str, Any]):
        ...
        logger.error(
            f"OVERDUE DEADLINE: {deadline['report_id']} was due on {deadline['deadline']}"
        )
    ...
    async def _generate_summary_report(self) -> Dict[str, Any]:
        ...
        return {
            "report_id": f"summary_{datetime.now(timezone.utc).strftime('%Y%m%d')}",
            "type": "summary",
        }
    ...
    async def _generate_risk_assessment_report(self) -> Dict[str, Any]:
        ...
        return {
            "report_id": f"risk_{datetime.now(timezone.utc).strftime('%Y%m%d')}",
    ...
    async def _generate_case_statistics_report(self) -> Dict[str, Any]:
        ...
        return {
            "report_id": f"cases_{datetime.now(timezone.utc).strftime('%Y%m%d')}",
            "type": "case_statistics",
        }
