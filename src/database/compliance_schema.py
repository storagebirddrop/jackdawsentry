...
class ComplianceSchemaManager:
        ...
        try:
            ...
            for expected in expected_constraints:
                if expected not in existing_constraints:
                    ...
                    verification_results["constraints"]["issues"].append(f"Missing constraint: {expected}")
        
        except Exception as e:
            verification_results["constraints"]["valid"] = False
            verification_results["constraints"]["issues"].append(f"Constraint verification failed: {e}")
        ...
        try:
            ...
            for expected in expected_indexes:
                if expected not in existing_indexes:
                    ...
                    verification_results["indexes"]["issues"].append(f"Missing index: {expected}")
        
        except Exception as e:
            verification_results["indexes"]["valid"] = False
            verification_results["indexes"]["issues"].append(f"Index verification failed: {e}")
        ...
        try:
            ...
            for check in orphaned_checks:
                ...
                if record and list(record.values())[0] > 0:
                    ...
                    verification_results["data_integrity"]["issues"].append(f"Orphaned nodes detected: {check}")
        
        except Exception as e:
            verification_results["data_integrity"]["valid"] = False
            verification_results["data_integrity"]["issues"].append(f"Data integrity check failed: {e}")
    ...
    async def get_schema_statistics(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        for rel_type, query in relationship_queries.items():
            try:
                result = await session.run(query)
                record = await result.single()
                statistics["relationship_counts"][rel_type] = record["count"] if record else 0
            except Exception as e:
    ...
    async def cleanup_old_data(self, session: AsyncSession, retention_days: int = 2555):
        ...
        try:
            ...
            record = await result.single()
            cleanup_stats["audit_events_deleted"] = record["deleted_count"] if record else 0
            ...
            """
            result = await session.run(compliance_query, cutoff_date=cutoff_date.isoformat())
            record = await result.single()
            cleanup_stats["compliance_logs_deleted"] = record["deleted_count"] if record else 0
