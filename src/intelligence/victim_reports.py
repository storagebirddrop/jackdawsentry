...
class VictimReportsDatabase:
            ...
            try:
                ...
                if result:
                    return VictimReport(
                        ...
                        amount_lost=float(result['amount_lost']) if result['amount_lost'] else None,
                        currency=result['currency'],
                        description=result['description'],
                        evidence=result['evidence'],
                        external_sources=result['external_sources'],
                        reported_date=result['reported_date'].isoformat() if result['reported_date'] else None,
    ...
    async def update_report_status(self, report_id: str, status: ReportStatus, resolved_date: Optional[datetime] = None) -> bool:
        ...
        async with get_postgres_connection() as conn:
            try:
                result = await conn.execute(update_query, status.value, resolved_date, report_id)
                
                success = result == "UPDATE 1"
                if success:
                    logger.info(f"Updated victim report {report_id} status to {status.value}")
    ...
    async def get_reports(self, filters: Dict[str, Any], limit: int = 100, offset: int = 0) -> List[VictimReport]:
    ...
    async def update_report(self, report_id: str, update_data: Dict[str, Any]) -> Optional[VictimReport]:
        ...
        for field, value in update_data.items():
            if field in ['status', 'severity', 'victim_address', 'scammer_address', 'scammer_entity', 
                       'platform', 'amount_lost', 'currency', 'description', 'evidence', 'external_sources',
                       'reported_date', 'incident_date', 'resolved_date', 'metadata']:
    ...
    async def get_statistics(self, start_date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        async with get_postgres_connection() as conn:
            try:
                ...
                false_positive_reports = sum(row['false_positive_reports'] for row in results)
                investigating_reports = sum(row['investigating_reports'] for row in results)
                ...
                return {
                    ...
                    'reports_by_type': {
                        row['report_type']: row['total_reports'] 
                        for row in results
                    },
                    'reports_by_severity': {
                        row['severity']: row['total_reports'] 
                        for row in results
                    },
                    ...
                    'average_amount_lost': float(total_amount_lost) / total_reports if total_reports > 0 else 0.0,
                    'verification_rate': verified_reports / total_reports if total_reports > 0 else 0.0,
                    'breakdown_by_type': [
                        {
