...
class BitcoinInvestigation:
    ...
    async def investigate_address(self, address: str) -> Dict[str, Any]:
        ...
        try:
                ...
                workflow["findings"].append(
                    {
                        ...
                        "risk_level": (
                            "high" if step3.result.data.get("count", 0) > 0 else "low"
                        ),
                        "confidence": 0.9,
                    }
                )
            ...
            workflow["risk_assessment"] = self._generate_risk_assessment(
                workflow["findings"]
            )
...
class OSINTWorkflowsManager:
    ...
    async def start_investigation(
        self, address: str, investigation_type: InvestigationType, user_id: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            if investigation_type == InvestigationType.BITCOIN_ADDRESS:
                ...
                async with bitcoin_investigator:
                    investigation = await bitcoin_investigator.investigate_address(
                        address
                    )
                ...
                async with ethereum_investigator:
                    investigation = await ethereum_investigator.investigate_address(
                        address
                    )
            ...
            logger.info(
                f"Started OSINT investigation for address {address}, type: {investigation_type.value}, ID: {investigation_id}"
            )
...
"""
    ...
    def _generate_recommendations(self, investigation: Dict[str, Any]) -> List[str]:
        ...
        if overall_risk == "critical":
            ...
            recommendations.extend(
                ["Standard monitoring sufficient", "Periodic risk assessment"]
            )
        ...
        for finding in findings:
            if finding.get("type") == "transaction_history":
                ...
                recommendations.append(
                    "Investigate clustered addresses for related activity"
                )
