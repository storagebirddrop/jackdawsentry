        ...
        try:
            ...
            payload = {
                'entities': [
                    {
                        'address': address,
                        'blockchain': 'ethereum'  # or bitcoin, etc.
                    }
                ]
            }
            ...
            async with self.session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Chainalysis API error: {response.status} - {error_text}")
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Chainalysis labels error: {response.status} - {error_text}")
        ...
        try:
            ...
            async with self.session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Elliptic API error: {response.status} - {error_text}")
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"CipherBlade API error: {response.status} - {error_text}")
        ...
        try:
            ...
            async with self.session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Arkham API error: {response.status} - {error_text}")
