...
class CrossPlatformAttributionEngine:
    ...
    async def get_cross_platform_attribution(
        ...
        try:
            ...
            # Get victim reports
            if not include_sources or AttributionSource.VICTIM_REPORTS in include_sources:
                victim_attributions = await self._get_victim_report_attributions(address, blockchain)
                all_attributions.extend(victim_attributions)
            
            # Get threat intelligence
            if not include_sources or AttributionSource.THREAT_INTELLIGENCE in include_sources:
                threat_attributions = await self._get_threat_intelligence_attributions(address, blockchain)
                all_attributions.extend(threat_attributions)
            
            # Get VASP registry attributions
            if not include_sources or AttributionSource.VASP_REGISTRY in include_sources:
                vasp_attributions = await self._get_vasp_registry_attributions(address, blockchain)
                all_attributions.extend(vasp_attributions)
            
            # Get on-chain analysis attributions
            if not include_sources or AttributionSource.ON_CHAIN_ANALYSIS in include_sources:
                onchain_attributions = await self._get_onchain_analysis_attributions(address, blockchain)
            ...
            logger.info(f"Cross-platform attribution complete for {address}: {consolidated.entity}")
            return consolidated
    ...
    async def _get_victim_report_attributions(self, address: str, blockchain: str) -> List[CrossPlatformAttribution]:
        ...
        try:
            ...
            for report in reports:
                if report.scammer_address and report.scammer_address.lower() == address.lower():
                    attribution = CrossPlatformAttribution(
                        id=str(hashlib.sha256(f"victim_reports:{address}:{report.report_id}").hexdigest()),
                        address=address,
                        blockchain=blockchain,
                        entity=report.scammer_entity,
                        entity_type="scammer",
                        confidence=self._calculate_confidence(report.status.value, report.severity.value),
                        sources=[{
                        ...
                        risk_score=self._calculate_risk_score(report.severity.value, report.amount_lost),
                        tags=["victim_report", "scam", "fraud"],
                        metadata={
                            "victim_reports": True,
                            "report_date": report.reported_date.isoformat() if report.reported_date else None,
    ...
    async def _get_threat_intelligence_attributions(self, address: str, blockchain: str) -> List[CrossPlatformAttribution]:
        """Get attributions from threat intelligence feeds"""
        
        try:
            # Search threat intelligence for this address
            threat_intelligence = await self.threat_intelligence_manager.search_intelligence(
                addresses=[address]
            )
            ...
            for intel in threat_intelligence:
                if intel.address.lower() == address.lower():
                    attribution = CrossPlatformAttribution(
                        id=str(hashlib.sha256(f"threat_intelligence:{address}:{intel.id}").hexdigest()),
                        address=address,
                        blockchain=blockchain,
                        entity=intel.entity,
                        entity_type="threat_actor",
                        confidence=self._calculate_confidence_from_threat_level(intel.threat_level.value, intel.confidence_score),
                        sources=[{
                        ...
                        risk_score=self._calculate_risk_score_from_threat_level(intel.threat_level.value),
    ...
    async def _get_vasp_registry_attributions(self, address: str, blockchain: str) -> List[CrossPlatformAttribution]:
            ...
            if result.attributions:
                for attribution in result.attributions:
                    vasp_attribution = CrossPlatformAttribution(
                        id=str(hashlib.sha256(f"vasp_registry:{address}:{attribution.id}").hexdigest()),
                        address=address,
                        blockchain=blockchain,
                        entity=attribution.vasp_id,
                        entity_type=attribution.entity_type.value if attribution.entity_type else "unknown",
                        confidence=self._convert_confidence(attribution.confidence_score),
                        sources=[{
    ...
    async def _get_onchain_analysis_attributions(self, address: str, blockchain: str) -> List[CrossPlatformAttribution]:
        """Get attributions from on-chain analysis"""
        
        try:
            # This would integrate with existing on-chain analysis
            # For now, return empty list as placeholder
            logger.debug(f"On-chain analysis attributions not yet implemented for {address}")
            return []
    ...
    def _consolidate_attributions(self, attributions: List[CrossPlatformAttribution]) -> AttributionConsolidation:
        ...
        overall_confidence = self._calculate_overall_confidence(attributions, best_score)
    ...
    def _calculate_entity_consensus_score(self, entity_attributions: List[CrossPlatformAttribution]) -> float:
    ...
    def _calculate_overall_confidence(self, attributions: List[CrossPlatformAttribution], consensus_score: float) -> AttributionConfidence:
        ...
        evidence_count = sum(len(attribution.evidence or []) for attribution in attributions)
        ...
        if final_confidence >= self.confidence_thresholds[AttributionConfidence.DEFINITIVE]:
            return AttributionConfidence.DEFINITIVE
        elif final_confidence >= self.confidence_thresholds[AttributionConfidence.VERY_HIGH]:
        ...
        elif final_confidence >= self.confidence_thresholds[AttributionConfidence.MEDIUM]:
    ...
    def _calculate_confidence(self, status: str, severity: str) -> AttributionConfidence:
    ...
    def _calculate_confidence_from_threat_level(self, threat_level: str, confidence_score: float) -> AttributionConfidence:
    ...
    def _convert_confidence(self, confidence_score: float) -> AttributionConfidence:
        ...
        if confidence_score >= self.confidence_thresholds[AttributionConfidence.DEFINITIVE]:
            return AttributionConfidence.DEFINITIVE
        elif confidence_score >= self.confidence_thresholds[AttributionConfidence.VERY_HIGH]:
            return AttributionConfidence.VERY_HIGH
    ...
    def _calculate_risk_score(self, severity: str, amount_lost: Optional[float] = None) -> float:
    ...
    async def batch_cross_platform_attribution(
        ...
        async def get_single_attribution(address: str) -> Tuple[str, Optional[CrossPlatformAttribution]]:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_risk_score = float(results[0]['avg_risk_score']) if results and results[0]['avg_risk_score'] else 0.0
            ...
            return {
                ...
                'total_consolidations': consolidation_results['total_consolidations'] if consolidation_results else 0,
                'avg_consolidation_score': float(consolidation_results['avg_consolidation_score']) if consolidation_results and consolidation_results['avg_consolidation_score'] else 0.0,
                'with_conflicts': consolidation_results['with_conflicts'] if consolidation_results else 0,
                'with_support': consolidation_results['with_support'] if consolidation_results else 0,
