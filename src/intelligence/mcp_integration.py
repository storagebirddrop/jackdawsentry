...
class EtherscanMCPServer:
    ...
    async def get_gas_price(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        url = (
            f"{self.base_url}?module=gastracker&action=gasoracle&apikey={self.api_key}"
        )
...
class AIInvestigationAssistant:
    ...
    async def investigate_address(self, address: str, depth: int = 3) -> Dict[str, Any]:
        ...
        try:
                ...
                investigation_result["analysis_steps"].append(
                    {
                        ...
                        "data": {
                            "transaction_count": len(
                                tx_result.data.get("transactions", [])
                            )
                        },
            ...
            investigation_result["risk_assessment"] = self._generate_risk_assessment(
                investigation_result["findings"]
            )

            # Step 5: Generate evidence report
            investigation_result["evidence"] = self._generate_evidence_report(
                investigation_result
            )
            ...
            return {
                "address": address,
                "investigation_id": investigation_result.get(
...
class MCPIntegration:
    ...
    async def execute_mcp_command(
        self, command: MCPCommandType, parameters: Dict[str, Any]
    ) -> MCPResponse:
        """Execute MCP command with full integration"""
        request_id = f"mcp_{datetime.now(timezone.utc).timestamp()}"
        request = MCPRequest(
            command=command, parameters=parameters, request_id=request_id
        )

        return await self.etherscan_server.execute_command(request)

    async def start_ai_investigation(
        self, address: str, user_id: str = None
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Started AI investigation for address {address}, ID: {investigation_id}"
        )
...
"""
        ...
        summary += (
            f" All data sources are cryptographically verifiable on the blockchain."
        )
