...
from .dark_web import DarkWebMonitor
from .dark_web import ThreatIndicator
from .dark_web import get_dark_web_monitor
from .sanctions import SanctionsManager
from .sanctions import SanctionsMatch
from .sanctions import get_sanctions_manager
...
class IntelligenceAggregator:
    ...
    def _initialize_sample_intelligence(self):
        ...
        sample_items = [
            ...
            },
            {
                ...
                "context": {
                    "report_title": "Crypto Crime Report",
                    "date": "2023-06-01",
                },
            },
        ]
    ...
    async def aggregate_intelligence(
        self, target_value: str, target_type: str = "address"
        ...
        try:
            ...
            intelligence_items = await self._collect_intelligence(
            ...
            overall_risk_score = self._calculate_overall_risk(
                intelligence_items, correlations
            )
            ...
            threat_types = [
                tag for sublist in threat_types for tag in sublist
            ]  # Flatten

            # Generate recommendations
            recommendations = self._generate_recommendations(
                intelligence_items, correlations, risk_level
            )

            # Create aggregated intelligence
            target_id = hashlib.sha256(
                f"{target_value}_{target_type}".encode()
            ).hexdigest()
    ...
    async def _collect_intelligence(
        self, target_value: str, target_type: str
        ...
        if sanctions_result.matches:
            for match in sanctions_result.matches:
                item = IntelligenceItem(
                    ...
                    tags=[match.list_type.value, "sanctions"]
                    + [p.value for p in match.programs],
                    context={
        ...
        for item in self.intelligence_items.values():
            if (
                ...
                and item.value.lower() == target_value.lower()
            ):
                intelligence_items.append(item)
    ...
    async def _correlate_items(
        self, item1: IntelligenceItem, item2: IntelligenceItem
    ) -> Optional[IntelligenceCorrelation]:
    ...
    def _calculate_overall_risk(
        ...
        item_risks = []
        for item in intelligence_items:
            source_weight = self.sources.get(item.source_type, {}).get(
                "severity_weight", 0.5
            )
    ...
    async def get_intelligence_summary(
        self, time_range: int = 24
    ) -> IntelligenceSummary:
        """Get intelligence summary for reporting"""
        try:
            # Calculate summary metrics
            total_targets = len(
                set(item.value for item in self.intelligence_items.values())
            )
            ...
            for item in self.intelligence_items.values():
                ...
                risk_level_distribution[risk_level] = (
                    risk_level_distribution.get(risk_level, 0) + 1
                )
        ...
        try:
            async with get_redis_connection() as redis:
                ...
                agg_dict = {
                    ...
                    "intelligence_items": [
                        item.__dict__ for item in aggregation.intelligence_items
                    ],
                    "correlations": [
                        corr.__dict__ for corr in aggregation.correlations
                    ],
                ...
                await redis.setex(
                    f"intelligence_agg:{key}", self.cache_ttl, json.dumps(agg_dict)
                )
        except Exception as e:
                ...
                if cached:
                    ...
                    return AggregatedIntelligence(
                        ...
                        intelligence_items=[
                            ...
                            for item in agg_dict["intelligence_items"]
                        ],
                        correlations=[
                            IntelligenceCorrelation(**corr)
                            for corr in agg_dict["correlations"]
                        ],
    ...
    async def cache_intelligence_data(self):
        ...
        try:
            async with get_redis_connection() as redis:
                ...
                await redis.setex(
                    "intelligence_stats", self.cache_ttl, json.dumps(stats)
                )
        except Exception as e:
