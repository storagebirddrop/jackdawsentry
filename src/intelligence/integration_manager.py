...
from .academic_research import AcademicResearchType
from .academic_research import get_academic_research
from .amrita_forensics import ForensicsMethodology
from .amrita_forensics import get_amrita_forensics
from .blokbustr_integration import get_blokbustr_integration
from .mcp_integration import MCPCommandType
from .mcp_integration import get_mcp_integration
from .osint_workflows import InvestigationType
from .osint_workflows import get_osint_workflows_manager
from .professional_tools import ProfessionalToolType
from .professional_tools import get_professional_tools_manager
...
class IntegrationManager:
    ...
    def _initialize_capabilities(self):
        ...
        logger.info(
            f"Initialized integration capabilities: {self.integration_status.capabilities}"
        )
    ...
    async def comprehensive_analysis(
        ...
        capabilities: List[IntegrationCapability] = None,
        user_id: str = None,
    ) -> IntegratedAnalysis:
        ...
        try:
            ...
            logger.info(
                f"Comprehensive analysis completed for {target}, ID: {analysis_id}, Time: {analysis.processing_time:.2f}s"
            )
    ...
    async def _execute_mcp_analysis(self, analysis: IntegratedAnalysis, target: str):
        ...
        try:
            ...
            investigation = await self.mcp_integration.start_ai_investigation(
                target, analysis.metadata.get("user_id")
            )
    ...
    async def _execute_professional_tools_analysis(
        ...
        try:
            ...
            if "combined_assessment" in tools_result:
                analysis.results["professional_risk_assessment"] = tools_result[
                    "combined_assessment"
                ]
    ...
    async def _execute_osint_analysis(self, analysis: IntegratedAnalysis, target: str):
        ...
        try:
            ...
            investigation = await self.osint_workflows.start_investigation(
                target, investigation_type, analysis.metadata.get("user_id")
            )
    ...
    async def _execute_academic_research(
        self, analysis: IntegratedAnalysis, target: str
    ):
        ...
        try:
            ...
            risk_assessment = {
                ...
                "sources_considered": list(
                    set(finding.get("source", "unknown") for finding in all_findings)
                ),
            ...
            logger.info(
                f"Risk assessment generated for {analysis.target}: {overall_risk}"
            )
    ...
    async def _collect_evidence(self, analysis: IntegratedAnalysis):
        ...
        try:
            ...
            for evidence_item in analysis.evidence:
                ...
                organized_evidence["evidence_by_type"][evidence_type].append(
                    evidence_item
                )
            ...
            logger.info(
                f"Evidence collection completed for {analysis.target}: {len(analysis.evidence)} items"
            )
    ...
    def _collect_all_findings(
        self, analysis: IntegratedAnalysis
    ) -> List[Dict[str, Any]]:
...
"""
    ...
    def _generate_source_citations(self, analysis: IntegratedAnalysis) -> List[str]:
        ...
        if IntegrationCapability.MCP_AI_ANALYSIS in analysis.capabilities_used:
            citations.append(
                "Model Context Protocol (MCP) - Real-time AI-powered blockchain analysis"
            )
        ...
        if IntegrationCapability.PROFESSIONAL_TOOLS in analysis.capabilities_used:
            ...
            if enabled_tools.get("chainalysis"):
                citations.append(
                    "Chainalysis Reactor - Professional blockchain intelligence platform"
                )
            if enabled_tools.get("elliptic"):
                citations.append(
                    "Elliptic - Advanced blockchain forensics and investigation"
                )
            if enabled_tools.get("cipherblade"):
                citations.append(
                    "CipherBlade - Blockchain security and investigation tools"
                )
            if enabled_tools.get("arkham"):
                citations.append(
                    "Arkham Intelligence - Entity graph analysis and visualization"
                )

        # OSINT workflows citations
        if IntegrationCapability.OSINT_WORKFLOWS in analysis.capabilities_used:
            citations.append(
                "Structured OSINT Workflows - Multi-platform investigation methodologies"
            )
            citations.append(
                "Bitcoin/Ethereum investigation patterns and best practices"
            )

        # Academic research citations
        if IntegrationCapability.ACADEMIC_RESEARCH in analysis.capabilities_used:
            citations.append(
                "Academic Research Integration - Peer-reviewed algorithms and methodologies"
            )
        ...
        citations.append(
            "Jackdaw Sentry Integrated Intelligence - Multi-platform blockchain analysis"
        )
...
law enforcement agencies, and compliance professionals.
    ...
    async def get_integration_status(self) -> IntegrationStatus:
        ...
        try:
            ...
            return IntegrationStatus(
                errors=[str(e)], timestamp=datetime.now(timezone.utc)
            )
    ...
    async def cleanup_old_analyses(self, days: int = 30):
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {len(old_analyses)} old analyses older than {days} days"
            )
