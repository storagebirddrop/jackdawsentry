    ...
    def rotate_log(self):
        ...
        try:
            if Path(self.filename).stat().st_size > self.max_bytes:
                # Rotate log file
                backup_path = (
                    f"{self.filename}.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
...
def setup_logging():
    ...
    audit_handler.addFilter(
        GDPRFilter(include_sensitive=True)
    )  # Include sensitive data for audit
    ...
    loggers = {
        ...
        "audit": {"level": logging.INFO, "handlers": ["audit"], "propagate": False},
        # Security logger
        "security": {
        ...
        # Database logger
        "database": {"level": logging.INFO, "handlers": ["file"], "propagate": False},
        # API logger
        "api": {"level": logging.INFO, "handlers": ["file"], "propagate": False},
        # Blockchain collectors logger
        "collectors": {"level": logging.INFO, "handlers": ["file"], "propagate": False},
        # Analysis engine logger
        "analysis": {"level": logging.INFO, "handlers": ["file"], "propagate": False},
    }
...
def log_with_context(logger: logging.Logger, level: int, message: str, **context):
    ...
    extra = {"timestamp": datetime.now(timezone.utc).isoformat(), **context}
...
def log_error_with_traceback(
    logger: logging.Logger, message: str, exception: Exception
):
