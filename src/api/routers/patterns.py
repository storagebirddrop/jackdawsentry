...
async def batch_analyze_patterns(
    ...
    *Requires bulk screening permission*
    ...
    try:
        ...
        total_patterns = sum(len(result.patterns) for result in results.values() if result)
...
async def list_patterns(
    pattern_types: Optional[List[PatternType]] = Query(None, description="Filter by pattern types"),
    min_severity: Optional[PatternSeverity] = Query(None, description="Filter by minimum severity"),
...
async def _get_database_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        stats["total_detections_24h"] = await conn.fetchval(queries["total_detections_24h"])
        
        # Total detections (7d)
        stats["total_detections_7d"] = await conn.fetchval(queries["total_detections_7d"])
        
        # High risk detections (24h)
        stats["high_risk_detections_24h"] = await conn.fetchval(queries["high_risk_detections_24h"])
        ...
        stats["most_detected_pattern"] = most_detected["pattern_id"] if most_detected else None
