...
VALID_CASE_STATUSES = {"open", "in_progress", "evidence_collection", "analysis", "review", "closed", "archived"}
VALID_EVIDENCE_TYPES = {"transaction_data", "blockchain_data", "address_analysis", "network_traffic", "communication_logs", "documents", "images", "videos", "audio", "metadata", "testimony", "expert_report", "other"}
VALID_EVIDENCE_INTEGRITY = {"verified", "tampered", "corrupted", "unknown"}
VALID_REPORT_TYPES = {"summary", "detailed", "expert_witness", "court_submission", "technical", "executive", "evidence_chain", "attribution"}
VALID_REPORT_FORMATS = {"pdf", "html", "json", "xml", "docx", "markdown"}
VALID_LEGAL_STANDARDS = {"preponderance", "clear_and_convincing", "beyond_reasonable_doubt", "reasonable_certainty"}
...
async def list_forensic_cases(
    ...
    assigned_investigator: Optional[str] = Query(None, description="Filter by investigator"),
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to list forensic cases",
            details=str(e)
        )
...
async def create_forensic_case(
    ...
    try:
        ...
        logger.info(f"Created forensic case {created_case.id} by user {current_user.id}")
        return ForensicCaseResponse(**created_case.__dict__)
        
    except Exception as e:
        logger.error(f"Failed to create forensic case: {e}")
        raise JackdawException(
            message="Failed to create forensic case",
            details=str(e)
        )
...
async def get_forensic_case(
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to get forensic case",
            details=str(e)
        )
...
async def update_forensic_case(
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to update forensic case",
            details=str(e)
        )
...
async def add_evidence_to_case(
    ...
    try:
        ...
        logger.info(f"Added evidence {created_evidence.id} to case {case_id} by user {current_user.id}")
...
@router.post("/reports/generate", response_model=ForensicReportResponse, status_code=status.HTTP_201_CREATED)
async def generate_forensic_report(
    ...
    try:
        ...
        logger.info(f"Started report generation for case {report_request.case_id} by user {current_user.id}")
...
async def get_forensic_report(
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to get forensic report",
            details=str(e)
        )
...
async def prepare_for_court(
    ...
    try:
        ...
        logger.info(f"Prepared court submission for case {case_id} by user {current_user.id}")
        ...
        raise JackdawException(
            message="Failed to prepare court submission",
            details=str(e)
        )
...
async def get_case_evidence(
    ...
    integrity_status: Optional[str] = Query(None, description="Filter by integrity status"),
    current_user: User = Depends(check_permissions(PERMISSIONS["read_intelligence"]))
):
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to get case evidence",
            details=str(e)
        )
...
async def get_forensics_statistics(
    ...
    try:
        ...
        raise JackdawException(
            message="Failed to get forensics statistics",
            details=str(e)
        )
