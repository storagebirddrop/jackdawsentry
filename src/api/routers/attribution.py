...
async def search_vasps(
    query: Optional[str] = Query(None, description="Search query for VASP name or description"),
...
async def _get_vasp_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        stats["by_entity_type"] = {row["entity_type"]: row["count"] for row in entity_rows}
...
async def _get_attribution_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        stats["by_blockchain"] = {row["blockchain"]: row["count"] for row in blockchain_rows}
        ...
        stats["by_status"] = {row["verification_status"]: row["count"] for row in status_rows}
...
async def _get_source_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            row["source_type"]: {
                "count": row["count"],
                "average_reliability": float(row["avg_reliability"]) if row["avg_reliability"] else 0.0
            }
            for row in rows
        }
    except Exception as e:
