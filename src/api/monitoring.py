...
from src.api.logging_config import log_error_with_traceback
...
class AlertManager:
    ...
    async def get_alert_history(self, limit: int = 100) -> List[Alert]:
        ...
        async with self._lock:
            return sorted(
                self.alerts.values(), key=lambda a: a.timestamp, reverse=True
            )[:limit]
...
class ErrorHandler:
    ...
    async def capture_exception(
        ...
        log_error_with_traceback(
            logger, f"Error captured: {error_event.error_type}", exception
        )
        return error_event
    ...
    async def get_error_summary(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            recent_errors = [
                e
                for e in self.error_events.values()
        ...
        for name, check_func in self.health_checks.items():
            try:
                ...
                await self.metrics_collector.record_timer(
                    f"health_check_duration", duration, {"check": name}
                )

                if result:
                    await self.metrics_collector.increment_counter(
                        "health_check_success", labels={"check": name}
                    )
                else:
                    await self.metrics_collector.increment_counter(
                        "health_check_failure", labels={"check": name}
                    )
                ...
                await self.metrics_collector.increment_counter(
                    "health_check_error", labels={"check": name}
                )
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                await monitoring_system.error_handler.capture_exception(
                    e,
                    {
                        "function": name,
                        "args": str(args)[:100],
