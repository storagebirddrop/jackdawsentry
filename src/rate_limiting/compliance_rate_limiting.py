...
class ComplianceRateLimitingEngine:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379"):
        ...
        self.default_limits = {
            ...
            "endpoint": {"requests": 50, "window": 60}     # 50 requests per minute per endpoint
        }
    ...
    async def _get_applicable_rules(self, request: RateLimitRequest) -> List[RateLimitRule]:
    ...
    async def _check_rule_conditions(self, rule: RateLimitRule, request: RateLimitRequest) -> bool:
        """Check if rule conditions are met"""
        try:
    ...
    async def _get_rate_limit_key(self, request: RateLimitRequest, rule: RateLimitRule) -> str:
            ...
            if self.redis_client:
                ...
                if key in self.request_history:
                    while (self.request_history[key] and 
                           self.request_history[key][0] < window_start):
    ...
    async def _log_violation(self, request: RateLimitRequest, rule: RateLimitRule, result: RateLimitResult):
        ...
        try:
            ...
            logger.warning(f"Rate limit violation: {rule.rule_id} for user {request.user_id} on {request.endpoint}")
            
        except Exception as e:
            logger.error(f"Failed to log violation: {e}")

    async def _take_action(self, request: RateLimitRequest, rule: RateLimitRule, result: RateLimitResult):
    ...
    async def get_rate_limit_status(self, user_id: Optional[str] = None, ip_address: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                ...
                user_status = {}
                for rule in self.rules.values():
                    if rule.enabled and rule.rate_limit_type == RateLimitType.USER_BASED:
    ...
    async def get_violations(self, limit: int = 100, user_id: Optional[str] = None) -> List[RateLimitViolation]:
    ...
    async def get_rate_limit_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_violations = len([v for v in self.violations if v.timestamp > now - timedelta(hours=1)])
            daily_violations = len([v for v in self.violations if v.timestamp > now - timedelta(days=1)])
            ...
            top_users = sorted(user_violations.items(), key=lambda x: x[1], reverse=True)[:10]
            top_ips = sorted(ip_violations.items(), key=lambda x: x[1], reverse=True)[:10]
