...
class MLClusteringEngine:
    ...
    async def extract_address_features(
        self, address: str, blockchain: str, time_range: int = 30
    ) -> AddressFeatures:
        ...
        try:
            ...
            if cache_key in self.feature_cache:
                ...
                if (
                    cached_time
                    and (datetime.now(timezone.utc) - cached_time).total_seconds()
                    < self.cache_ttl
                ):
            ...
            total_received = sum(
                tx["value"] for tx in transactions if tx["to_address"] == address
            )
            total_sent = sum(
                tx["value"] for tx in transactions if tx["from_address"] == address
            )
            ...
            round_amount_transactions = len(
                [tx for tx in transactions if self._is_round_amount(tx["value"])]
            )
            off_peak_transactions = len(
                [tx for tx in transactions if self._is_off_peak(tx["timestamp"])]
            )
            high_frequency_periods = await self._count_high_frequency_periods(
                transactions
            )
            cross_chain_activity = await self._check_cross_chain_activity(
                address, blockchain
            )
            cluster_connections = await self._count_cluster_connections(
                address, blockchain
            )
            ...
            features = AddressFeatures(
                ...
                metadata={
                    "amount_variance": (
                        statistics.variance(amounts) if len(amounts) > 1 else 0
                    ),
    ...
    async def _get_address_transactions(
        self, address: str, blockchain: str, time_range: int
    ) -> List[Dict]:
        ...
        async with get_neo4j_session() as session:
            result = await session.run(
                query, address=address, blockchain=blockchain, time_range=time_range
            )
            transactions = []
    ...
    async def calculate_risk_score(self, features: AddressFeatures) -> RiskAssessment:
        ...
        try:
            ...
            feature_scores["privacy_tool_usage"] = (
                0.8 if features.privacy_tool_usage else 0.0
            )
            feature_scores["cross_chain_activity"] = (
                0.3 if features.cross_chain_activity else 0.0
            )
            ...
            logger.error(
                f"Error calculating risk score for address {features.address}: {e}"
            )
            return RiskAssessment(
                address=features.address,
                blockchain=features.blockchain,
    ...
    def _calculate_confidence(
        self, features: AddressFeatures, feature_scores: Dict[str, float]
    ) -> float:
            ...
            for cluster_id, cluster_addresses in clusters.items():
                if len(cluster_addresses) >= self.min_cluster_size:
                    cluster = await self._create_cluster(
                        cluster_id, cluster_addresses, blockchain, features_dict
                    )
    ...
    async def _calculate_similarity_matrix(
        self, features_dict: Dict[str, AddressFeatures]
            ...
            for j, addr2 in enumerate(addresses):
                if i == j:
                    ...
                    similarity = await self._calculate_similarity(
                        features_dict[addr1], features_dict[addr2]
                    )
    ...
    async def _calculate_similarity(
        self, features1: AddressFeatures, features2: AddressFeatures
    ) -> float:
        ...
        if features1.transaction_count > 0 and features2.transaction_count > 0:
            freq_similarity = 1 - abs(
                features1.avg_transaction_frequency
                - features2.avg_transaction_frequency
            ) / max(
                features1.avg_transaction_frequency,
                features2.avg_transaction_frequency,
                1,
        ...
        if features1.total_sent > 0 and features2.total_sent > 0:
            amount_similarity = 1 - abs(
                features1.avg_transaction_amount - features2.avg_transaction_amount
            ) / max(
                features1.avg_transaction_amount, features2.avg_transaction_amount, 1
            )
        ...
        if features1.active_days > 0 and features2.active_days > 0:
            temporal_similarity = 1 - abs(
                features1.active_days - features2.active_days
            ) / max(features1.active_days, features2.active_days)
        ...
        if features1.unique_counterparties > 0 and features2.unique_counterparties > 0:
            counterparty_similarity = 1 - abs(
                features1.unique_counterparties - features2.unique_counterparties
            ) / max(features1.unique_counterparties, features2.unique_counterparties)
    ...
    async def _perform_clustering(
        self, addresses: List[str], similarity_matrix: Dict[str, Dict[str, float]]
    ) -> Dict[str, List[str]]:
        ...
        for address in addresses:
            ...
            for other_addr in addresses:
                if other_addr == address:
                    continue
                if (
                    other_addr not in assigned
                    and similarity_matrix[address][other_addr]
    ...
    async def _create_cluster(
        ...
        addresses: List[str],
        blockchain: str,
        features_dict: Dict[str, AddressFeatures],
    ) -> AddressCluster:
        """Create cluster object"""
        # Filter features_dict to only cluster members
        cluster_features = {
            addr: features_dict[addr] for addr in addresses if addr in features_dict
        }
        ...
        avg_risk_score = sum(
            features.metadata.get("risk_score", 0)
            for features in cluster_features.values()
    ...
    async def _identify_cluster_characteristics(
        ...
        avg_frequency = statistics.mean(
            [f.avg_transaction_frequency for f in features_dict.values()]
        )
        if avg_frequency > 10:
        ...
        # Analyze volume
        total_volume = sum(
            f.total_sent + f.total_received for f in features_dict.values()
        )
    ...
    async def get_address_risk_assessment(
        self, address: str, blockchain: str, time_range: int = 30
    ) -> RiskAssessment:
        """Get comprehensive risk assessment for address"""
        try:
            # Extract features
            features = await self.extract_address_features(
                address, blockchain, time_range
            )
            ...
            cluster_affiliation = await self._get_address_cluster_affiliation(
                address, blockchain
            )
    ...
    async def _get_address_cluster_affiliation(
