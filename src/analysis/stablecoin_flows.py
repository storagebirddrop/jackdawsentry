...
class StablecoinFlowTracker:
    ...
    async def track_stablecoin_flow(
        self, tx_hash: str, blockchain: str
    ) -> Optional[StablecoinFlow]:
        """Track stablecoin flow for a specific transaction"""
        try:
            # Analyze the transaction
            tx_analysis = await self.cross_chain_analyzer.analyze_transaction(
                tx_hash, blockchain
            )
            if not tx_analysis:
                return None
    ...
    async def _is_stablecoin_transaction(self, tx_analysis) -> bool:
        ...
        return tx_analysis.token_symbol in self.supported_stablecoins or any(
            pattern == TransactionPattern.STABLECOIN_FLOW
            for pattern in tx_analysis.patterns
        )
    ...
    async def _calculate_flow_duration(
    ...
    async def _calculate_flow_risk_score(
        self, flow_path: List[Dict[str, Any]], flow_type: FlowType
    ) -> float:
        ...
        duration_hours = (
            await self._calculate_flow_duration(flow_path).total_seconds() / 3600
        )
        ...
        suspicious_count = sum(
            1 for step in flow_path if step["type"] in suspicious_types
        )
    ...
    async def _calculate_flow_confidence(
        self, flow_path: List[Dict[str, Any]]
    ) -> float:
    ...
    async def _extract_flow_metadata(
        self, flow_path: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if len(flow_path) > 1:
            ...
            for i in range(1, len(flow_path)):
                hop_time = (
                    flow_path[i]["timestamp"] - flow_path[i - 1]["timestamp"]
                ).total_seconds()
    ...
    async def analyze_stablecoin_flows(
        ...
        try:
            ...
            for symbol, txs in stablecoin_groups.items():
                flow_analysis[symbol] = await self._analyze_stablecoin_group_flows(
                    address, symbol, txs
                )

            # Calculate overall metrics
            total_flows = sum(
                analysis["total_flows"] for analysis in flow_analysis.values()
            )
            total_amount = sum(
                analysis["total_amount"] for analysis in flow_analysis.values()
            )
            avg_risk_score = (
                sum(
            ...
            for analysis in flow_analysis.values():
                for flow_type, count in analysis["flow_type_distribution"].items():
                    flow_type_counts[flow_type] = (
                        flow_type_counts.get(flow_type, 0) + count
                    )
    ...
    async def _get_stablecoin_transactions(
        ...
        async with get_neo4j_session() as session:
            ...
            async for record in result:
                ...
                stablecoin_info = await self._get_transaction_stablecoin(
                    record["tx_data"]["hash"]
                )
                if stablecoin_info:
    ...
    async def _analyze_stablecoin_group_flows(
        self, address: str, symbol: str, transactions: List[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": (
                sum(flow.confidence for flow in flows) / len(flows) if flows else 0
            ),
        }
    ...
    async def get_bridge_flow_analysis(
        self, bridge_name: str, time_range: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with get_neo4j_session() as session:
                result = await session.run(
                    query,
                    blockchains=list(bridge_contracts.keys()),
        ...
        for chain, bridges in tracker.bridge_contracts.items():
            if bridge_key in bridges:
                ...
                if isinstance(contracts, dict):
                    addr = contracts.get("token_bridge") or next(
    ...
    async def cache_flow_analysis(self, key: str, result: Dict[str, Any]):
        ...
        try:
            async with get_redis_connection() as redis:
                await redis.setex(
                    f"stablecoin_flow:{key}", self.cache_ttl, json.dumps(result)
                )
        except Exception as e:
