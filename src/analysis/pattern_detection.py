...
from .cross_chain import TransactionPattern
...
class MLPatternType(Enum):
    ...
    CIRCULAR_TRADING = (
        "circular_trading"  # Circular transactions to create false volume
    )
...
class MLPatternDetector:
    ...
    async def detect_patterns(
        self, address: str, blockchain: str, time_range: int = 24
    ) -> List[PatternMatch]:
        ...
        try:
            ...
            pattern_matches.extend(
                await self._detect_synchronized_transfers(transactions)
            )
            pattern_matches.extend(
                await self._detect_rapid_chain_switching(transactions)
            )
    ...
    async def _detect_structuring(self, transactions: List[Dict]) -> List[PatternMatch]:
        ...
        for hour, hour_txs in hourly_groups.items():
            ...
            if len(hour_txs) >= 3:
                ...
                if total_amount > self.structuring_threshold and all(
                    tx["value"] < self.structuring_max_amount for tx in hour_txs
                ):
    ...
    async def _detect_integration(self, transactions: List[Dict]) -> List[PatternMatch]:
        ...
        for tx in transactions:
            if tx["to_address"] in legitimate_services:
                ...
                previous_txs = [
                    t for t in transactions if t["timestamp"] < tx["timestamp"]
                ]

                if len(previous_txs) >= 2:
                    # Calculate risk based on previous activity
                    risk_results = await asyncio.gather(
                        *(self._calculate_transaction_risk(t) for t in previous_txs)
                    )
    ...
    async def _detect_circular_trading(
        ...
        for path in circular_paths:
            if len(path) >= 3:
                ...
                circularity_score = (
                    1.0 - (amount_std / statistics.mean(amounts))
    ...
    async def _detect_mixer_usage(self, transactions: List[Dict]) -> List[PatternMatch]:
        ...
        mixer_checks = await asyncio.gather(
            *(self._is_mixer_transaction(tx) for tx in transactions)
        )
    ...
    async def _detect_privacy_tool_usage(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
        ...
        privacy_txs = [
            tx for tx in transactions if await self._is_privacy_tool_transaction(tx)
        ]
    ...
    async def _detect_bridge_hopping(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
        ...
        bridge_txs = [
            tx for tx in transactions if await self._is_bridge_transaction(tx)
        ]
    ...
    async def _detect_high_frequency(
        ...
        for hour, hour_txs in hourly_groups.items():
            if len(hour_txs) > self.high_frequency_threshold:
                total_amount = sum(tx["value"] for tx in hour_txs)
                confidence = min(
                    len(hour_txs) / (self.high_frequency_threshold * 2), 1.0
                )
    ...
    async def _detect_peak_off_hours(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
    ...
    async def _detect_synchronized_transfers(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
    ...
    async def _detect_rapid_chain_switching(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
        ...
        if len(transactions) >= 2:
            for i in range(1, len(transactions)):
                ...
                if (
                    prev_tx["blockchain"] != curr_tx["blockchain"]
                    and (curr_tx["timestamp"] - prev_tx["timestamp"])
                        ...
                        evidence=[
                            {
                                ...
                                "time_difference": (
                                    curr_tx["timestamp"] - prev_tx["timestamp"]
                                ).total_seconds(),
                                "transactions": [
                                    {
                                        "hash": prev_tx["hash"],
                                        "blockchain": prev_tx["blockchain"],
                                    },
                                    {
                                        "hash": curr_tx["hash"],
                                        "blockchain": curr_tx["blockchain"],
                                    },
                                ],
                            }
                        ],
    ...
    async def _detect_splitting_merging(
        self, transactions: List[Dict]
    ) -> List[PatternMatch]:
        ...
        for address, addr_txs in address_txs.items():
            if len(addr_txs) >= 3:
                ...
                if (
                    amount_std / amount_mean < 0.1 if amount_mean > 0 else 0
                ):  # Low variance
    ...
    # Helper methods
    def _group_transactions_by_hour(
        self, transactions: List[Dict]
    ...
    def _find_transaction_chains(self, transactions: List[Dict]) -> List[List[Dict]]:
        ...
        for i in range(len(transactions) - 2):
            ...
            if (
                chain[0]["to_address"] == chain[1]["from_address"]
                and chain[1]["to_address"] == chain[2]["from_address"]
            ):
                chains.append(chain)
        return chains
    ...
    async def _update_address_profile(
        self, address: str, blockchain: str, pattern_matches: List[PatternMatch]
    ):
        ...
        profile = AddressPatternProfile(
            ...
            risk_score=(
                sum(match.risk_score for match in pattern_matches)
                / len(pattern_matches)
                if pattern_matches
                else 0
            ),
                    ...
                    [
                        {
                            ...
                            "detected_at": (
                                ...
                                if isinstance(match.detected_at, datetime)
                                else str(match.detected_at)
                            ),
    ...
    ) -> Optional[AddressPatternProfile]:
                ...
                for m in raw_matches or []:
                    try:
                        deserialized_matches.append(
                            PatternMatch(
                                pattern_type=(
                                    MLPatternType(m["pattern_type"])
                                    if m.get("pattern_type")
                                    else MLPatternType.STRUCTURING
                                ),
                                confidence=m.get("confidence", 0.0),
                                risk_score=m.get("risk_score", 0.0),
                                evidence=m.get("evidence", []),
                                detected_at=(
                                    datetime.fromisoformat(m["detected_at"])
                                    if m.get("detected_at")
                        ...
                        logger.warning(
                            "Failed to deserialize PatternMatch: %s — pattern_type=%s",
                            exc,
                            m.get("pattern_type", "unknown"),
                        )
    ...
    async def cache_pattern_analysis(self, key: str, result: Dict[str, Any]):
        ...
        try:
            async with get_redis_connection() as redis:
                await redis.setex(
                    f"ml_pattern:{key}", self.cache_ttl, json.dumps(result)
                )
        except Exception as e:
