...
from .cross_chain import get_cross_chain_analyzer
from .mixer_detection import MixerDetector
from .mixer_detection import get_mixer_detector
from .ml_clustering import MLClusteringEngine
from .ml_clustering import get_ml_clustering_engine
from .pattern_detection import MLPatternDetector
from .pattern_detection import get_ml_pattern_detector
from .stablecoin_flows import StablecoinFlowTracker
from .stablecoin_flows import get_stablecoin_flow_tracker
...
class AnalysisManager:
    ...
    async def analyze_address(
        ...
        try:
            ...
            if "cross_chain" in self.engines:
                cross_chain_analysis = await self.engines[
                    "cross_chain"
                ].get_cross_chain_analysis(address, time_range)
                analysis_results["cross_chain_analysis"] = cross_chain_analysis

            # Stablecoin flow analysis
            if "stablecoin_flows" in self.engines:
                stablecoin_flows = await self.engines[
                    "stablecoin_flows"
                ].analyze_stablecoin_flows(address, time_range)
                analysis_results["stablecoin_flows"] = stablecoin_flows

            # ML pattern detection
            if "pattern_detection" in self.engines:
                ml_patterns = await self.engines["pattern_detection"].detect_patterns(
            ...
            if "mixer_detection" in self.engines:
                mixer_analysis = await self.engines[
                    "mixer_detection"
                ].detect_mixer_usage(address, blockchain, time_range)
                analysis_results["mixer_analysis"] = {
                    ...
                    "mixers_used": [
                        mixer.value for mixer in mixer_analysis.mixers_used
                    ],
            ...
            if "mixer_detection" in self.engines:
                privacy_analysis = await self.engines[
                    "mixer_detection"
                ].detect_privacy_tool_usage(address, blockchain, time_range)
                analysis_results["privacy_tool_analysis"] = privacy_analysis

            # ML risk assessment
            if "ml_clustering" in self.engines:
                risk_assessment = await self.engines[
                    "ml_clustering"
                ].get_address_risk_assessment(address, blockchain, time_range)
            ...
            # Cluster information
            if "ml_clustering" in self.engines and analysis_results[
                "risk_assessment"
            ].get("cluster_affiliation"):
            ...
            # Cache results
            await self.cache_analysis_results(
                f"address_{address}_{blockchain}", analysis_results
            )
    ...
    async def analyze_transaction(
        ...
        try:
            ...
            # Cross-chain analysis
            if "cross_chain" in self.engines:
                cross_chain_tx = await self.engines["cross_chain"].analyze_transaction(
                    tx_hash, blockchain
                )
                if cross_chain_tx:
                    analysis_results["cross_chain_analysis"] = {
                        "patterns": [
                            pattern.value for pattern in cross_chain_tx.patterns
                        ],
            ...
            if "stablecoin_flows" in self.engines:
                stablecoin_flow = await self.engines[
                    "stablecoin_flows"
                ].track_stablecoin_flow(tx_hash, blockchain)
                if stablecoin_flow:
            ...
            # Cache results
            await self.cache_analysis_results(
                f"transaction_{tx_hash}_{blockchain}", analysis_results
            )
    ...
    async def analyze_transaction_flow(
        self, start_address: str, end_address: str, blockchain: str
    ) -> Dict[str, Any]:
        """Analyze transaction flow between addresses"""
        try:
            logger.info(
                f"Analyzing transaction flow from {start_address} to {end_address}"
            )
            ...
            if "cross_chain" in self.engines:
                transaction_flow = await self.engines[
                    "cross_chain"
                ].analyze_transaction_flow(start_address, end_address)
                if transaction_flow:
                    flow_analysis = {
                        ...
                        "patterns": [
                            pattern.value for pattern in transaction_flow.patterns
                        ],
            ...
            # Cache results
            await self.cache_analysis_results(
                f"flow_{start_address}_{end_address}_{blockchain}", flow_analysis
            )
    ...
    async def get_system_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "mixer_detection" in self.engines:
                mixer_stats = await self.engines[
                    "mixer_detection"
                ].get_mixer_statistics(24)
    ...
    async def cache_analysis_results(
        self, key: str, results: Dict[str, Any], ttl: int = 3600
    ):
        ...
        try:
            ...
            async with get_redis_connection() as redis:
                await redis.setex(
                    f"analysis:{safe_key}", ttl, json.dumps(results, default=str)
                )
        except Exception as e:
    ...
    async def cache_metrics(self):
        ...
        try:
            async with get_redis_connection() as redis:
                await redis.setex(
                    "analysis_metrics", 300, json.dumps(self.metrics)  # 5 minutes
                )
        except Exception as e:
    ...
    async def analyze_transfer_patterns(self, transfers: List[Dict]) -> Dict:
        ...
        try:
            ...
            sorted_bridges = sorted(
                bridge_volumes.items(), key=lambda x: x[1], reverse=True
            )
