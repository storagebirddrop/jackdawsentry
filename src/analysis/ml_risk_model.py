...
async def list_weights() -> List[Dict[str, Any]]:
    try:
        ...
        return [
            {"feature_name": k, "weight": v[0], "description": v[1]}
...
def extract_features(
    ...
    fv.privacy_tool_usage = (
        1.0 if getattr(address_features, "privacy_tool_usage", False) else 0.0
    )
    fv.cross_chain_activity = (
        1.0 if getattr(address_features, "cross_chain_activity", False) else 0.0
    )
    ...
    if entity_info:
        ...
        fv.sanctions_entity = (
            1.0 if entity_info.get("risk_level") == "critical" else 0.0
        )
    ...
    fv.low_counterparty_ratio = max(
        0.0, 1.0 - min(unique_cp / max(tx_count * 0.1, 1), 1.0)
    )
...
async def list_custom_rules() -> List[Dict[str, Any]]:
    try:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM custom_risk_rules ORDER BY created_at DESC"
            )
