...
from .models import (
    ...
    VASP, AttributionSource, VerificationStatus, EntityType
)
from .vasp_registry import get_vasp_registry
from .confidence_scoring import get_confidence_scorer, Evidence, EvidenceType
from src.api.database import get_postgres_connection, get_neo4j_session
...
class AttributionEngine:
    ...
    async def attribute_address(
        ...
        try:
            ...
            filtered_attributions = [
                attr for attr in consolidated.attributions 
            ...
            if include_evidence and filtered_attributions:
                source_ids = list(set(attr.attribution_source_id for attr in filtered_attributions))
            ...
            logger.info(f"Attribution complete for {address}: {len(filtered_attributions)} attributions")
            return result
    ...
    async def batch_attribute_addresses(
        ...
        logger.info(f"Batch attribution complete: {len(attribution_results)} successful, {len(failed_addresses)} failed")
        
        return attribution_results
    
    async def _find_exact_matches(self, address: str, blockchain: str) -> List[AddressAttribution]:
    ...
    async def _find_cluster_attributions(self, address: str, blockchain: str) -> List[AddressAttribution]:
    ...
    async def _analyze_on_chain_patterns(self, address: str, blockchain: str) -> List[AddressAttribution]:
    ...
    async def _consolidate_attributions(
        ...
        for vasp_id, attrs in vasp_attributions.items():
            ...
            primary_attr.corroborating_sources = list(all_sources - {primary_attr.attribution_source_id})
            ...
            if len(attrs) > 1:
                ...
                primary_attr.confidence_score = min(1.0, primary_attr.confidence_score * 1.1)
            
            consolidated_attributions.append(primary_attr)
        
        # Calculate overall confidence score
        overall_confidence = max(attr.confidence_score for attr in consolidated_attributions) if consolidated_attributions else 0.0
    ...
    async def _get_attribution_sources(self, source_ids: List[int]) -> List[AttributionSource]:
