...
class ComplianceBackupEngine:
    ...
    async def _process_backup(self, job: BackupJob):
        ...
        try:
            ...
            backup_filename = f"compliance_backup_{job.config.backup_type.value}_{timestamp}"
    ...
    async def _create_full_backup(self, job: BackupJob, backup_path: Path):
        ...
        try:
            ...
            # Create tar archive
            with tarfile.open(backup_path, 'w:gz' if job.config.compression else 'w') as tar:
                for source_path in job.config.source_paths:
                    source = Path(source_path)
                    if source.exists():
                        await self._add_to_tar(tar, source, job.config.exclude_patterns or [])
        ...
        try:
            ...
            with tarfile.open(backup_path, 'w:gz' if job.config.compression else 'w') as tar:
                for file_path in modified_files:
                    file = Path(file_path)
                    if file.exists():
                        await self._add_to_tar(tar, file, job.config.exclude_patterns or [])
    ...
    async def _add_to_tar(self, tar: tarfile.TarFile, source: Path, exclude_patterns: List[str]):
    ...
    async def _verify_backup_integrity(self, backup_path: Path):
        ...
        try:
            ...
            if backup_path.suffix == '.gz':
                ...
                if not self.encryption_key:
                    raise ValueError("Cannot verify encrypted backup without encryption key")
    ...
    async def _find_last_backup(self, backup_type: BackupType) -> Optional[BackupJob]:
        ...
        try:
            matching_backups = [
                ...
                if job.config.backup_type == backup_type and job.status == BackupStatus.COMPLETED
            ]
    ...
    async def _get_modified_files(self, source_paths: List[str], since: datetime) -> List[str]:
    ...
    async def restore_backup(self, backup_file: str, target_path: str, recovery_type: RecoveryType) -> RecoveryJob:
        """Restore from backup"""
        try:
            # Generate unique job ID
    ...
    async def _process_recovery(self, job: RecoveryJob):
        ...
        try:
            ...
            if backup_path.suffix == '.enc':
                if not self.encryption_key:
                    raise ValueError("Cannot restore encrypted backup without encryption key")
            ...
            logger.info(f"Recovery completed: {job.job_id} ({len(job.recovered_files)} files)")
    ...
    async def _decrypt_backup(self, encrypted_path: Path) -> Path:
        ...
        try:
            ...
            decrypted_path = encrypted_path.with_suffix(encrypted_path.suffix.replace('.enc', ''))
    ...
    async def _decompress_backup(self, compressed_path: Path) -> Path:
        ...
        try:
            decompressed_path = compressed_path.with_suffix(compressed_path.suffix.replace('.gz', ''))
    ...
    async def list_backups(self, backup_type: Optional[BackupType] = None, limit: int = 50) -> List[BackupJob]:
    ...
    async def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_size = sum(b.file_size or 0 for b in self.backup_history if b.file_size)
    ...
    async def start_backup_scheduler(self):
        ...
        while True:
            try:
                ...
                for backup in self.backup_history:
                    if (backup.config.schedule_interval_hours > 0 and 
