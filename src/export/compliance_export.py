...
class ComplianceExportEngine:
    ...
    async def create_export(self, request: ExportRequest) -> ExportResult:
        ...
        try:
            ...
            if request.export_id in self.active_exports:
                ...
                self.active_exports[request.export_id].completed_at = datetime.now(
                    timezone.utc
                )
        ...
        try:
            ...
            for report in reports:
                report_data = {
                    ...
                    "content": (
                        report.content if request.include_sensitive else "REDACTED"
                    ),
                }
        ...
        try:
            ...
            for case in cases:
                case_data = {
                    ...
                    "evidence_count": (
                        len(case.evidence) if hasattr(case, "evidence") else 0
                    ),
                }
                ...
                if request.include_sensitive and hasattr(ev, "content"):
                    ...
                    if content_size < 1000000:  # 1MB limit
                        ...
                        evidence_data["content"] = (
                            f"CONTENT_TOO_LARGE ({content_size} bytes)"
                        )
    ...
    async def _format_export_data(
        self, request: ExportRequest, data: Union[List[Dict], Dict]
    ) -> str:
        """Format data based on requested format"""
    ...
    async def _export_to_json(
        self, data: Union[List[Dict], Dict], filename: str
    ) -> str:
    ...
    async def _export_to_pdf(self, data: Union[List[Dict], Dict], filename: str) -> str:
        ...
        try:
            ...
            async with aiofiles.open(text_file_path, "w", encoding="utf-8") as f:
                ...
                await f.write(
                    f"Record Count: {len(data) if isinstance(data, list) else 1}\n"
                )
                await f.write("\n" + "=" * 50 + "\n")
    ...
    async def delete_export(self, export_id: str) -> bool:
        ...
        try:
            ...
            # Remove from history
            self.export_history = [
                r for r in self.export_history if r.export_id != export_id
            ]
