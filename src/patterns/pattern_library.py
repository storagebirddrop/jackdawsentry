...
from .models import PatternIndicator
...
class PatternLibrary:
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        for pattern in patterns:
            ...
            stats["patterns_by_type"][ptype] = (
                stats["patterns_by_type"].get(ptype, 0) + 1
            )

        # Count by severity
        for pattern in patterns:
            severity = pattern.severity.value
            stats["patterns_by_severity"][severity] = (
                stats["patterns_by_severity"].get(severity, 0) + 1
            )

        # Calculate averages
        if patterns:
            stats["avg_indicators_per_pattern"] = sum(
                len(p.indicators) for p in patterns
            ) / len(patterns)
            stats["avg_time_window_hours"] = sum(
                p.time_window_hours for p in patterns
            ) / len(patterns)
