...
class LayeringDetector:
    ...
    async def detect_layering(
        ...
        if len(transactions) < self.min_hops:
            return self._create_empty_result(
                "Insufficient transactions for layering analysis"
            )
        ...
        # Analyze best path
        best_path = max(
            layering_paths, key=lambda path: self._calculate_path_confidence(path)
        )
    ...
    def _build_transaction_graph(
        ...
        while queue and len(visited) < 100:  # Limit to prevent infinite loops
            ...
            outgoing_txs = [
                tx for tx in transactions if tx.sender.lower() == current_addr.lower()
            ]
    ...
    def _find_layering_paths(
        self, graph: Dict[str, List[Transaction]], start_address: str
    ...
    def _calculate_path_confidence(self, path: List[Transaction]) -> float:
        ...
        diversity_bonus = min(
            0.2, (unique_counterparties - self.min_unique_counterparties) * 0.05
        )
    ...
    def _has_similar_amounts(self, path: List[Transaction]) -> bool:
        ...
        similar_count = sum(
            1 for amount in amounts if abs(amount - avg_amount) / avg_amount <= 0.1
        )
    ...
    def _create_evidence(
        self, path: List[Transaction], address: str
