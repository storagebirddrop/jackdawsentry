...
class CustodyChangeDetector:
    ...
    async def detect_custody_change(
        ...
        if len(transactions) < self.min_transactions:
            return self._create_empty_result("Insufficient transactions for custody analysis")
        ...
        # Create evidence
        evidence = self._create_custody_evidence(sorted_txs, address, behavior_change, inactivity_change, amount_change)
    ...
    def _analyze_behavior_change(self, transactions: List[Transaction]) -> float:
        ...
        if first_variance > 0 and second_variance > 0:
            variance_change = abs(first_variance - second_variance) / max(first_variance, second_variance)
        ...
        diversity_change = abs(first_diversity - second_diversity) / max(first_diversity, second_diversity, 1)
    ...
    def _create_custody_evidence(self, transactions: List[Transaction], address: str, 
                                behavior_change: float, inactivity_change: float, amount_change: float) -> List[PatternEvidence]:
...
class SynchronizedTransferDetector:
    ...
    async def detect_synchronized_transfers(
        ...
        if len(transactions) < self.min_transfers:
            return self._create_empty_result("Insufficient transactions for sync analysis")
        ...
        # Analyze best group
        best_group = max(sync_groups, key=lambda group: self._calculate_group_confidence(group))
        ...
        return PatternResult(
            ...
            evidence=evidence,
            indicators_met=["timing_synchronization", "amount_similarity", "multiple_addresses"],
            indicators_missed=[],
    ...
    def _find_synchronized_groups(self, transactions: List[Transaction]) -> List[List[Transaction]]:
    ...
    def _determine_severity(self, confidence: float, group_size: int) -> PatternSeverity:
    ...
    def _create_sync_evidence(self, group: List[Transaction], address: str) -> List[PatternEvidence]:
...
class OffPeakActivityDetector:
    ...
    async def detect_off_peak_activity(
        ...
        if len(transactions) < self.min_transactions:
            return self._create_empty_result("Insufficient transactions for off-peak analysis")
        ...
        # Create evidence
        evidence = self._create_off_peak_evidence(transactions, address, off_peak_ratio, high_value_off_peak)
    ...
    def _create_off_peak_evidence(self, transactions: List[Transaction], address: str,
                                 off_peak_ratio: float, high_value_off_peak: float) -> List[PatternEvidence]:
        ...
        if high_value_off_peak > 0:
            evidence.append(PatternEvidence(
                ...
                metadata={
                    ...
                    "high_value_count": int(high_value_off_peak * len(transactions))
                }
            ))
...
class RoundAmountDetector:
    ...
    async def detect_round_amounts(
        ...
        if len(transactions) < self.min_transactions:
            return self._create_empty_result("Insufficient transactions for round amount analysis")
    ...
    def _find_round_amount_transactions(self, transactions: List[Transaction]) -> List[Transaction]:
        ...
        for tx in round_txs:
            for round_amount in self.round_amounts:
                if abs(tx.amount - round_amount) <= round_amount * self.tolerance:
                    amount_counts[str(round_amount)] = amount_counts.get(str(round_amount), 0) + 1
    ...
    def _determine_severity(self, confidence: float, round_count: int) -> PatternSeverity:
