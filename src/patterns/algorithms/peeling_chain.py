...
class PeelingChainDetector:
    ...
    async def detect_peeling_chain(
        ...
        best_sequence = max(
            peeling_sequences, key=lambda seq: self._calculate_sequence_confidence(seq)
        )
        ...
        if best_sequence[-1].timestamp - best_sequence[0].timestamp > timedelta(
            hours=self.max_time_gap_hours
        ):
        ...
        return PatternResult(
            ...
            metadata={
                ...
                "time_span_hours": (
                    best_sequence[-1].timestamp - best_sequence[0].timestamp
                ).total_seconds()
                / 3600,
    ...
    def _find_peeling_sequences(
        self, transactions: List[Transaction], address: str
    ...
    def _calculate_sequence_confidence(self, sequence: List[Transaction]) -> float:
        ...
        # Total confidence
        confidence = (
            base_confidence + length_bonus + consistency_bonus + compression_bonus
        )
    ...
    def _determine_severity(
        self, confidence: float, sequence_length: int
    ) -> PatternSeverity:
    ...
    def _create_evidence(
        self, sequence: List[Transaction], address: str
        ...
        if len(sequence) >= 2:
            ...
            for i in range(1, len(sequence)):
                if sequence[i - 1].amount > 0:
                    ratio = (sequence[i - 1].amount - sequence[i].amount) / sequence[
                        i - 1
                    ].amount
            ...
            evidence.append(
                PatternEvidence(
                    ...
                    metadata={
                        ...
                        "decrease_consistency": self._calculate_amount_consistency(
                            sequence
                        ),
                    },
                )
        ...
        evidence.append(
            PatternEvidence(
                ...
                metadata={
                    ...
                    "time_compression_score": self._calculate_time_compression(
                        sequence
                    ),
                },
            )
