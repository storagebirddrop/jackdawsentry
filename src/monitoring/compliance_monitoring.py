...
class ComplianceMonitoringEngine:
    ...
    async def _evaluate_alert_rule(
        self, rule: Dict[str, Any], metric: ComplianceMetric
    ) -> bool:
    ...
    async def _check_rate_threshold(
        self, rule: Dict[str, Any], metric: ComplianceMetric
    ) -> bool:
    ...
    async def _check_ratio_threshold(
        self, rule: Dict[str, Any], metric: ComplianceMetric
    ) -> bool:
        ...
        try:
            if "cache_hits" in metric.name:
                ...
                if hits_metric and misses_metric:
                    hits = 0
                    misses = 0
                    for metric_family in prometheus.REGISTRY.collect():
                        for sample in metric_family.samples:
                            if sample.name == "compliance_cache_hits_total":
                                hits += sample.value
                            elif sample.name == "compliance_cache_misses_total":
                                misses += sample.value
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    self.alert_webhook_url, json=payload, timeout=10
                ) as response:
    ...
    async def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "active_alerts": len(
                    [a for a in self.alerts.values() if a.status == AlertStatus.FIRING]
                ),
