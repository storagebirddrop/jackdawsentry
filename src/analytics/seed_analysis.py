...
from .models import DerivationType
from .models import SeedAnalysisRequest
from .models import SeedAnalysisResult
from .models import WalletDerivation
...
class SeedPhraseAnalyzer:
    ...
    async def analyze_seed_phrase(
        self, request: SeedAnalysisRequest
    ) -> SeedAnalysisResult:
        """
        ...
        try:
            ...
            if not request.include_inactive:
                all_derivations = [
                    ...
                    if d.balance > 0 or d.transaction_count > 0
                ]

            # Calculate statistics
            total_balance = sum(d.balance or 0.0 for d in all_derivations)
            active_wallets = len(
                [d for d in all_derivations if d.balance > 0 or d.transaction_count > 0]
            )
            ...
            logger.info(
                f"Seed phrase analysis complete: {len(all_derivations)} wallets derived in {processing_time:.2f}ms"
            )
            return result
    ...
    async def _derive_wallets(
        ...
        try:
            ...
            if derivation_type == DerivationType.BIP44:
                derivations = await self._derive_bip44_wallets(
                    master_key, blockchain, max_derivations
                )
            elif derivation_type == DerivationType.BIP49:
                derivations = await self._derive_bip49_wallets(
                    master_key, blockchain, max_derivations
                )
            elif derivation_type == DerivationType.BIP84:
                derivations = await self._derive_bip84_wallets(
                    master_key, blockchain, max_derivations
                )
            elif derivation_type == DerivationType.BIP32:
                derivations = await self._derive_bip32_wallets(
                    master_key, blockchain, max_derivations
                )
            elif derivation_type == DerivationType.CUSTOM:
                derivations = await self._derive_custom_wallets(
                    master_key, blockchain, max_derivations
                )
            ...
            try:
                ...
                for address_index in range(20):  # 20 addresses per account
                    try:
                        ...
                        address = self._generate_address_from_key(
                            address_key, blockchain, "P2PKH"
                        )
                ...
                for address_index in range(10):  # 10 change addresses per account
                    try:
                        ...
                        logger.debug(
                            f"Error deriving change address {address_index}: {e}"
                        )
                        continue
            ...
            try:
                ...
                for address_index in range(20):
                    try:
                        ...
                        logger.debug(
                            f"Error deriving BIP49 address {address_index}: {e}"
                        )
                        continue
            ...
            try:
                ...
                for address_index in range(20):
                    try:
                        ...
                        logger.debug(
                            f"Error deriving BIP84 address {address_index}: {e}"
                        )
                        continue
            ...
            try:
                ...
                for address_index in range(20):
                    try:
                        ...
                        logger.debug(
                            f"Error deriving BIP32 address {address_index}: {e}"
                        )
                        continue
            ...
            try:
                for address_index in range(min(50, max_derivations)):
                    try:
                        ...
                        address = self._generate_address_from_key(
                            address_key, blockchain, address_type
                        )
                        ...
                        logger.debug(
                            f"Error deriving custom address {address_index}: {e}"
                        )
                        continue
    ...
    def _generate_address_from_key(
        self, key: bip_utils.Bip32PrivateKey, blockchain: str, address_type: str
    ) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error generating address for {blockchain} ({address_type}): {e}"
            )
            raise
